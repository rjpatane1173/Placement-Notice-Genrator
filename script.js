...
function generatePdf() {
  ...
  doc.setFontSize(14);
  doc.setFont("helvetica", "bold");
  doc.text("PLACEMENT NOTICE", centerX, margin + 38, { align: 'center' });
  ...
  formattedNotice.forEach(line => {
    ...
    if (line.toUpperCase() === line && line.match(/[A-Z]/)) {
      ...
      doc.text(lines, margin, yPosition, { align: 'left' });
      ...
      doc.setFont("helvetica", "bold");
      doc.text(key + ':', margin, yPosition, { align: 'left' });
      doc.setFont("helvetica", "normal");
      const valueLines = doc.splitTextToSize(value.trim(), contentWidth - 30);
      doc.text(valueLines, margin + 30, yPosition, { align: 'left' });
      yPosition += Math.max(1, valueLines.length) * lineHeight;
    } else {
      // Regular text
      const lines = doc.splitTextToSize(line, contentWidth);
      doc.text(lines, margin, yPosition, { align: 'left' });
  ...
  doc.text("Generated by Placement Cell, Indira Group of Institutes", centerX, pageHeight - margin, { align: 'center' });
